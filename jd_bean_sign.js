...
let cookiesArr = [];
let notification = "";
const stopVar = process.env.JD_BEAN_STOP
  ? process.env.JD_BEAN_STOP
  : "2000-5000";
...
*************************/
      ...
      return new Promise((resolve) => {
        try {
          ...
          $nobyda.notify(
            "",
            "",
            Name + one + two + three + four + five + disa,
            {
              "media-url":
                $nobyda.headUrl ||
    ...
    (async function ReadCookie() {
      ...
      if (DeleteCookie) {
        ...
        stop =
          Wait($nobyda.read("JD_DailyBonusDelay"), true) || Wait(stop, true);
        boxdis =
            ...
            try {
              if (error) {
                ...
                if (cc.code == 3) {
                  ...
                  merge.JDTurn.notify =
                    "京东商城-转盘: 失败, 原因: Cookie失效‼️";
                  merge.JDTurn.fail = 1;
                } else if (data.match(/(\"T216\"|活动结束)/)) {
                  merge.JDTurn.notify =
                    "京东商城-转盘: 失败, 原因: 活动结束 ⚠️";
                  merge.JDTurn.fail = 1;
            ...
            try {
              ...
              if (
                ...
                if (
                  ...
                  merge.JRSteel.notify =
                    "京东金融-钢镚: 失败, 原因: Cookie失效‼️";
                } else {
                  const ng =
              ...
              if (error) {
                ...
                if (data.match(/prize/)) {
                  ...
                  if (cc.data.prizeBean) {
                    ...
                    merge.JDShake.notify += `${
                      also ? `\n` : ``
                    }京东商城-摇摇: ${also ? `多次` : `成功`}, 明细: ${
                      merge.JDShake.bean || `无`
                    }京豆 🐶`;
                  } else if (cc.data.prizeCoupon) {
                    merge.JDShake.notify += `${
                      also ? `\n` : ``
                    }京东商城-摇摇: ${also ? `多次, ` : ``}获得满${
                      cc.data.prizeCoupon.quota
                    }减${cc.data.prizeCoupon.discount}优惠券→ ${
              ...
              if (error) {
                ...
                if (cc.result.code == 0) {
                  ...
                  if (data.match(/已参与|已领取|\"2006\"/)) {
                    ...
                    data.match(/不存在|已结束|未开始|\"2008\"|\"2012\"/)
                  ) {
                    merge.JDFSale.notify =
          ...
          (error, response, data) => {
            try {
              ...
              if (cc.msg == "success" && cc.retcode == 0) {
                merge.TotalBean.nickname =
                  cc.data.userInfo.baseInfo.nickname || "";
      ...
      if (annal && way == 1 && boxdis) {
        ...
        if (read) {
          if (read.indexOf(Val) == -1) {
            var Crash = $nobyda.write(
              `${read},${Val}`,
              "JD_DailyBonusDisables"
            );
            console.log(`\n${name}-触发自动禁用 ‼️`);
    ...
    function Wait(readDelay, ini) {
      ...
      if (typeof readDelay == "string") {
        ...
        const plan = parseInt(
          Math.random() * (raw[1] - raw[0] + 1) + raw[0],
          10
        );
        if (ini)
          console.log(
    ...
    function nobyda() {
      ...
      const notify = (title, subtitle, message, rawopts) => {
        ...
        if (isSurge)
          $notification.post(title, subtitle, message, Opts(rawopts));
        if (isJSBox)
          $push.schedule({
            title: title,
