      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.status) {
              console.log(
                `签到成功，获得${data.data.point}，已签到${data.data.signDays}天`
              );
...
function doMission(missionId) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("v1/game/mission", { missionId: missionId }),
      async (err, resp, data) => {
        try {
          if (err) {
            data = JSON.parse(resp.body);
...
function receiveMission(missionId) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("v1/user/mission/receive", { missionId: missionId }),
      async (err, resp, data) => {
        try {
          if (err) {
            data = JSON.parse(resp.body);
...
function taskUrl(function_id, body = {}) {
  return {
    ...
    headers: {
      ...
      Referer:
        "https://h5.m.jd.com/babelDiy/Zeus/44bjzCpzH9GpspWeBzYSqBA7jEtP/index.html",
      Cookie: cookie,
      "User-Agent": $.isNode()
