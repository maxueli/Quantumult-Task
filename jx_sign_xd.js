...
[task_local]
#京喜签到-喜豆
50 1,8 * * * https://raw.githubusercontent.com/he1pu/JDHelp/main/jx_sign_xd.js, tag=京喜签到-喜豆, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "50 1,8 * * *" script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jx_sign_xd.js,tag=京喜签到-喜豆

===============Surge=================
京喜签到-喜豆 = type=cron,cronexp="50 1,8 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jx_sign_xd.js

============小火箭=========
京喜签到-喜豆 = type=cron,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jx_sign_xd.js, cronexpr="50 1,8 * * *", timeout=3600, enable=true
 */
!(function (t, r) {
...
function signhb() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("signhb/query", "type=0&signhb_source=5&smp=&ispp=1&tk="),
      async (err, resp, data) => {
        try {
          if (err) {
...
function dotask(task) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        "signhb/dotask",
        `task=${task}&signhb_source=5&ispp=1&sqactive=${$.sqactive}&tk=`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
...
function taskUrl(functionId, body = "") {
  ...
  if (body) {
    ...
    url = `${JD_API_HOST}fanxiantask/${functionId}?_=${
      Date.now() + 2
    }&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(
      Math.floor(Math.random() * 26) + "A".charCodeAt(0)
    )}&g_ty=ls`;
