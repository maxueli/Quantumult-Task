...
async function pasture() {
  try {
    ...
    if (JSON.stringify($.homeInfo) === "{}") {
      ...
      console.log(
        `打印新手流程进度：当前进度：${$.currentStep}，下一流程：${$.homeInfo?.maintaskId}`
      );
      if ($.homeInfo?.maintaskId !== "pause" || isNew($.currentStep)) {
        console.log(`开始初始化`);

        $.step = isNew($.currentStep)
          ? isNew($.currentStep, true)
          : $.homeInfo?.maintaskId;
        ...
        for (let vo of $.GetCardInfo.cardinfo) {
          ...
          if (petNum < 6) {
            ...
            for (let i = vo.currnum; i >= vo.neednum; i -= vo.neednum) {
              console.log(
                `${cardinfo[vo.cardtype]}卡片已满${vo.neednum}张，去兑换...`
              );
              await $.wait(5000);
      ...
      for (let i = 0; i < petNum; i++) {
        ...
        switch (stage) {
          case 1:
            console.log(
              `这里有一只幼年${petInfo[type].name}，成长进度：${progress}%`
            );
            break;
          case 2:
            console.log(
              `这里有一只青年${petInfo[type].name}，生蛋进度：${bornvalue}/${strong}，成长进度：${progress}%`
            );
            break;
          case 4:
            console.log(
              `这里有一只壮年${petInfo[type].name}，离家出走进度（生蛋进度）：${bornvalue}/${strong}`
            );
            break;
          default:
            console.log(
              `这里有一只不知道什么状态的鸡：${JSON.stringify($.onepetInfo)}`
            );
        }
...
async function doTask(j) {
  for (let i = 0; i < $.taskList.length; i++) {
    ...
    if ($.oneTask.dateType === 1) {
      //成就任务
      if (
        $.oneTask.awardStatus === 2 &&
        $.oneTask.completedTimes === $.oneTask.targetTimes
      ) {
      ...
      if ($.oneTask.awardStatus === 1) {
        ...
        //浏览任务
        if (
          Number($.oneTask.completedTimes) > 0 &&
          $.oneTask.completedTimes === $.oneTask.targetTimes
        ) {
          console.log(`完成任务：${$.oneTask.taskName}`);
          await takeGetRequest("Award");
        ...
        for (
          let j = Number($.oneTask.completedTimes);
          j < Number($.oneTask.configTargetTimes);
          j++
        ) {
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case "GetUserTaskStatusList":
      url = `https://m.jingxi.com/newtasksys/newtasksys_front/GetUserTaskStatusList?_=${
        Date.now() + 2
      }&source=${$.source}&bizCode=${$.bizCode}&dateType=${
        $.dateType
      }&showAreaTaskFlag=0&jxpp_wxapp_type=7`;
      url += `&_stk=${getStk(url)}`;
      url += `&_ste=1&h5st=${decrypt(
        Date.now(),
        "",
        "",
        url
      )}&sceneval=2&g_login_type=1&g_ty=ajax`;
    ...
    case "jump":
      ...
      break;
    case "DoTask":
      url = `https://m.jingxi.com/newtasksys/newtasksys_front/DoTask?_=${
        Date.now() + 2
      }&source=${$.source}&taskId=${$.oneTask.taskId}&bizCode=${
        $.bizCode
      }&configExtra=`;
      url += `&_stk=${getStk(url)}`;
      url += `&_ste=1&h5st=${decrypt(
      ...
      break;
    case "Award":
      url = `https://m.jingxi.com/newtasksys/newtasksys_front/Award?_=${
        Date.now() + 2
      }&source=${$.source}&taskId=${$.oneTask.taskId}&bizCode=${$.bizCode}`;
      url += `&_stk=${getStk(url)}`;
      url += `&_ste=1&h5st=${decrypt(
        Date.now(),
        "",
      ...
      break;
    case "cow":
      url = `https://m.jingxi.com/jxmc/operservice/GetCoin?channel=7&sceneid=1001&activeid=${
        $.activeid
      }&activekey=${$.activekey}&token=${A(
        $.crowInfo.lastgettime
      )}&jxmc_jstoken=${token["farm_jstoken"]}&timestamp=${
        token["timestamp"]
      }&phoneid=${token["phoneid"]}&_stk=channel%2Csceneid%2Ctoken&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), "", "", url)}&_=${
        Date.now() + 2
      }&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(
        Math.floor(Math.random() * 26) + "A".charCodeAt(0)
...
function dealReturn(type, data) {
  switch (type) {
    ...
    case "cow":
      ...
      if (data && data[1]) {
        ...
        if (data.ret === 0) {
          ...
          let add =
            $.mowingInfo.addcoins || $.mowingInfo.addcoin
              ? $.mowingInfo.addcoins || $.mowingInfo.addcoin
              : 0;
    ...
    case "Award":
      ...
      if (data.ret === 0) {
        console.log(
          `领取金币成功，获得${JSON.parse(data.data.prizeInfo).prizeInfo}`
        );
      }
      break;
    case "buy":
    ...
    case "GetEgg":
      ...
      if (data.ret === 0) {
        console.log(
          `成功收取${data.data.addnum}个蛋，现有鸡蛋${data.data.newnum}个`
        );
      }
      break;
    case "DoTask":
    ...
    case "BuyNew":
      ...
      if (data.ret === 0) {
        ...
        $.coins -= costcoin;
        console.log(
          `获得一只${petInfo[type].name}，宠物id：${petid}，当前拥有${currnum}只鸡`
        );
      } else {
    ...
    case "DrawLoveHongBao":
      ...
      if (data.ret === 0) {
        console.log(
          `领取爱心奖励获得：${data.data.rewardinfo.prizevalue / 100}红包`
        );
      } else {
...
function getAuthorShareCode(
  url = "https://raw.githubusercontent.com/he1pu/params/main/jxmc.json"
) {
  return new Promise(async (resolve) => {
    const options = {
      url: `${url}?${new Date()}`,
...
function readShareCode() {
  return new Promise(async (resolve) => {
    $.get(
      { url: `https://transfer.nz.lu/jxmc`, timeout: 30 * 1000 },
      (err, resp, data) => {
        try {
          if (err) {
            console.log(JSON.stringify(err));
...
function uploadShareCode(code) {
  return new Promise(async (resolve) => {
    $.post(
      {
        url: `https://transfer.nz.lu/upload/jxmc?code=${code}&ptpin=${encodeURIComponent(
...
function getJxToken() {
  ...
  return new Promise((_0x2ef875) => {
    ...
    let _0x4e1006 = $.cookie[_0x5108("a", "8#od")](
      /pt_pin=([^; ]+)(?=;?)/
    )[0x1];
    let _0x57bff6 = $["md5"](
      "" +
