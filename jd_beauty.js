...
async function accountCheck() {
  ...
  let client = new WebSocket(
    `wss://xinruimz-isv.isvjcloud.com/wss/?token=${$.token}`,
    null,
    {
      headers: {
        "user-agent": $.isNode()
...
async function mr() {
  ...
  client.onclose = () => {
    ...
    for (let i = 0; i < $.pos.length && i < $.tokens.length; ++i) {
      $.helpInfo.push(
        `{"msg":{"type":"action","args":{"inviter_id":"${$.userInfo.id}","position":"${$.pos[i]}","token":"${$.tokens[i]}"},"action":"employee"}}`
      );
    }
  };
  client.onmessage = async function (e) {
      ...
      switch (vo.action) {
        case "get_ad":
          ...
          if (vo.data.check_sign_in === 1) {
            ...
            console.log(`去做签到任务`);
            client.send(
              `{"msg":{"type":"action","args":{},"action":"sign_in"}}`
            );
            client.send(
              `{"msg":{"action":"write","type":"action","args":{"action_type":1,"channel":2,"source_app":2}}}`
            );
          }
          break;
        case "get_user":
          ...
          if ($.userInfo.newcomer === 0) {
            ...
            for (let i = $.userInfo.step; i < 15; ++i) {
              client.send(
                `{"msg":{"type":"action","args":{},"action":"newcomer_update"}}`
              );
              await $.wait(500);
            }
          } else $.init = true;
        ...
        case "shop_products":
          ...
          for (let i = 0; i < vo.data.shops.length && count < 5; ++i) {
            ...
            if (!$.taskState.shop_view.includes(shop.id)) {
              ...
              client.send(
                `{"msg":{"type":"action","args":{"shop_id":${shop.id}},"action":"shop_view"}}`
              );
              client.send(
                `{"msg":{"action":"write","type":"action","args":{"action_type":6,"channel":2,"source_app":2,"vender":"${shop.vender_id}"}}}`
              );
            }
            await $.wait(1000);
          }
          ...
          for (
            let i = 0;
            i < vo.data.products.length && count < 5 && ADD_CART;
            ++i
          ) {
            ...
            if (!$.taskState.product_adds.includes(product.id)) {
              ...
              client.send(
                `{"msg":{"type":"action","args":{"add_product_id":${product.id}},"action":"add_product_view"}}`
              );
              client.send(
                `{"msg":{"action":"write","type":"action","args":{"action_type":9,"channel":2,"source_app":2,"vender":"${product.id}"}}}`
              );
              client.send(
                `{"msg":{"action":"write","type":"action","args":{"action_type":5,"channel":2,"source_app":2,"vender":"${product.id}"}}}`
              );
            }
            await $.wait(1000);
          }
          for (
            let i = $.taskState.meetingplace_view;
            i < $.taskState.mettingplace_count;
            ++i
          ) {
            console.log(`去做第${i + 1}次浏览会场任务`);
            client.send(
              `{"msg":{"type":"action","args":{"source":1},"action":"meetingplace_view"}}`
            );
            await $.wait(2000);
          }
          if ($.taskState.today_answered === 0) {
            ...
            client.send(
              `{"msg":{"type":"action","args":{"source":1},"action":"get_question"}}`
            );
          }
          break;
        case "check_up":
          ...
          for (let check_up of vo.data.check_up) {
            if (check_up["receive_status"] !== 1) {
              ...
              client.send(
                `{"msg":{"type":"action","args":{"check_up_id":${check_up.id}},"action":"check_up_receive"}}`
              );
            } else {
        ...
        case "newcomer_update":
          if (vo.code === "200" || vo.code === 200) {
            console.log(
              `第${vo.data.step}步新手任务完成成功，获得${vo.data.coins}美妆币`
            );
        ...
        case "get_question":
          ...
          await $.wait(5000);
          client.send(
            `{"msg":{"type":"action","args":{"commit":${JSON.stringify(
              commit
            )},"correct":${questions.length}},"action":"submit_answer"}}`
          );
          break;
        case "complete_task":
        case "action":
        ...
        case "produce_position_info_v2":
          ...
          if (vo.data && vo.data.material_name !== "") {
            console.log(
              `【${vo?.data?.position}】上正在生产【${vo?.data?.material_name}】，可收取 ${vo.data.produce_num} 份`
            );
            if (new Date().getTime() > vo.data.procedure.end_at) {
              console.log(`去收取${vo?.data?.material_name}`);
              client.send(
                `{"msg":{"type":"action","args":{"position":"${vo?.data?.position}","replace_material":false},"action":"material_fetch_v2"}}`
              );
              client.send(
                `{"msg":{"type":"action","args":{},"action":"to_employee"}}`
              );
            ...
            if (vo?.data && vo.data.valid_electric > 0) {
              ...
              if ($.needs.length) {
                ...
                ma =
                  $.material.base[0]["items"][
                    positionList.indexOf(vo.data.position)
                  ];
              ...
              if (ma) {
                ...
                client.send(
                  `{"msg":{"type":"action","args":{"position":"${vo.data.position}","material_id":${ma.id}},"action":"material_produce_v2"}}`
                );
              } else {
                ma =
                  $.material.base[1]["items"][
                    positionList.indexOf(vo.data.position)
                  ];
                if (ma) {
        ...
        case "material_produce_v2":
          console.log(
            `【${vo?.data?.position}】上开始生产${vo?.data?.material_name}`
          );
          client.send(
            `{"msg":{"type":"action","args":{},"action":"to_employee"}}`
          );
        ...
        case "material_fetch_v2":
          if (vo.code === "200" || vo.code === 200) {
            console.log(
              `【${vo.data.position}】收取成功，获得${vo.data.procedure.produce_num}份${vo.data.material_name}\n`
            );
          } else {
            console.log(`任务完成失败，错误信息${vo.msg}`);
          }
          ...
          if (vo.code === "200" || vo.code === 200) {
            ...
            for (let product of $.products) {
              ...
              for (let material of product.product_materials) {
                ...
                const ma = $.materials.filter(
                  (vo) => vo.item_id === material.material_id
                )[0]; //仓库里对应的材料信息
                // console.log(`ma:${JSON.stringify(ma)}`);
                if (ma) {
                  msg += `（库存 ${ma.num} 份）`;
                  ...
                  if (
                    need_material.findIndex(
                      (vo) => vo.id === material.material.id
                    ) === -1
                  )
                    need_material.push(material.material);
              ...
              if (num !== Infinity && num > 0) {
                ...
                client.send(
                  `{"msg":{"type":"action","args":{"product_id":${product.id},"amount":${num}},"action":"product_produce"}}`
                );
                await $.wait(500);
              } else {
                console.log(msg);
        ...
        case "collect_coins":
          if (vo.code === "200" || vo.code === 200) {
            // console.log(`product_produce:${JSON.stringify(vo)}`)
            console.log(
              `收取成功，获得${vo["data"]["coins"]}美妆币，当前总美妆币：${vo["data"]["user_coins"]}\n`
            );
          } else {
          ...
          if (vo.code === "200" || vo.code === 200) {
            for (let product of vo.data) {
              if (product.num === product.produce_num) {
                client.send(
                  `{"msg":{"type":"action","args":{"log_id":${product.id}},"action":"new_product_fetch"}}`
                );
              } else {
                console.log(
                  `产品【${product.product.id}】未生产完成，无法收取`
                );
              }
            }
          } else {
            ...
            if (benefit.type === 1) {
              ...
              if (
                ...
                for (let i = benefit.day_exchange_count; i < 10; i++) {
                  // console.log(`开始兑换`)
                  client.send(
                    `{"msg":{"type":"action","args":{"benefit_id":${benefit.id}},"action":"to_exchange"}}`
                  );
                  await $.wait(1000);
                }
              }
        ...
        case "to_exchange":
          if (vo?.data) {
            console.log(
              `兑换${vo?.data?.coins / -100}京豆成功;${JSON.stringify(vo)}`
            );
          } else {
