    ...
    $.get(taskUrl(`user/GetPropCardCenterInfo`), async (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log(
            `${$.name} GetPropCardCenterInfo API请求失败，请检查网路重试`
          );
        } else {
          data = JSON.parse(
...
function UsePropCard(strCardTypeIndex) {
  return new Promise((resolve) => {
    ...
    $.get(
      taskUrl(
        `user/UsePropCard`,
        `dwCardType=${dwCardType}&strCardTypeIndex=${encodeURIComponent(
          strCardTypeIndex
        )}`
      ),
      (err, resp, data) => {
...
function TreasureHunt(strIndex) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`user/TreasureHunt`, `strIndex=${strIndex}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (data.iRet === 0) {
              if (data.AwardInfo.dwAwardType === 0) {
                console.log(
                  `${data.strAwardDesc}，获得 ${data.AwardInfo.ddwValue} 金币`
                );
              } else if (data.AwardInfo.dwAwardType === 1) {
                console.log(
                  `${data.strAwardDesc}，获得 ${data.AwardInfo.ddwValue} 财富`
                );
                console.log(JSON.stringify(data));
              } else if (data.AwardInfo.dwAwardType === 4) {
                console.log(
                  `${data.strAwardDesc}，获得 ${data.AwardInfo.strPrizePrice} 红包`
                );
              } else {
                console.log(JSON.stringify(data));
              }
  ...
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        ...
        `strStoryId=${strStoryId}&dwType=${dwType}&triggerType=0&ddwTriggerDay=${ddwTriggerDay}`
      ),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} SpecialUserOper API请求失败，请检查网路重试`
            );
          } else {
            data = JSON.parse(
              data
            ...
            if (dwType === "2") {
              if (data.iRet === 0 || data.sErrMsg === "success") {
                ...
                console.log(
                  `贵宾'${StoryList.Special.strName}'下船失败 ${data.sErrMsg}\n`
                );
              }
            } else if (dwType === "3") {
              if (data.iRet === 0 || data.sErrMsg === "success") {
                console.log(
                  `贵宾'${StoryList.Special.strName}'用餐成功：获得${StoryList.Special.ddwCoin}金币\n`
                );
              } else {
                console.log(
                  `贵宾'${StoryList.Special.strName}'用餐失败：${data.sErrMsg}\n`
                );
              }
            }
          }
...
function collectorOper(strStoryId, dwType, ddwTriggerDay) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        `story/CollectorOper`,
        `strStoryId=${strStoryId}&dwType=${dwType}&ddwTriggerDay=${ddwTriggerDay}`
      ),
      (err, resp, data) => {
        try {
          if (err) {
          ...
          if (err) {
            ...
            switch (dwType) {
              ...
              case "3":
                if (data.iRet === 0 || data.sErrMsg === "success") {
                  dwType = "2";
                  let mermaidOperRes = await mermaidOper(
                    strStoryId,
                    dwType,
                    ddwTriggerDay
                  );
              ...
              case "4":
                if (data.iRet === 0 || data.sErrMsg === "success") {
                  console.log(
                    `昨日解救美人鱼领奖成功：获得${data.Data.Prize.strPrizeName}\n`
                  );
                } else {
...
async function getTakeAggrPage(type) {
  return new Promise(async (resolve) => {
    switch (type) {
        ...
        $.get(taskUrl(`story/GetTakeAggrPage`), async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
              console.log(
                `${$.name} GetTakeAggrPage API请求失败，请检查网路重试`
              );
            } else {
              data = JSON.parse(
                data
      ...
      case "wxsign":
        $.get(
          taskUrl(`story/GetTakeAggrPage`, "", 6),
          async (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
...
function helpdraw(dwUserId) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`story/helpdraw`, `dwUserId=${dwUserId}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (data.iRet === 0 || data.sErrMsg === "success") {
              if (data.Data.StagePrizeInfo) {
                console.log(
                  `领取助力奖励成功：获得${data.Data.ddwCoin}金币 ${
                    data.Data.StagePrizeInfo.ddwMoney
                  }财富 ${
                    data.Data.StagePrizeInfo.strPrizeName &&
                    !data.Data.StagePrizeInfo.ddwMoney
                      ? data.Data.StagePrizeInfo.strPrizeName
                      : `0元`
                  }红包`
              ...
              if (vo.Rubbish) {
                ...
                if (Object.keys(rubbishOperRes.Data.ThrowRubbish.Game).length) {
                  ...
                  for (let key of Object.keys(
                    rubbishOperRes.Data.ThrowRubbish.Game.RubbishList
                  )) {
                    let vo =
                  ...
                  if (rubbishOperTwoRes.iRet === 0) {
                    let AllRubbish =
                      rubbishOperTwoRes.Data.RubbishGame.AllRubbish;
                    console.log(
                      `倒垃圾成功：获得${AllRubbish.ddwCoin}金币 ${AllRubbish.ddwMoney}财富\n`
                    );
                  } else {
                  ...
                  if (
                    rubbishOperRes.iRet === 0 ||
                    rubbishOperRes.sErrMsg === "success"
                  ) {
                    console.log(
                      `倒垃圾成功：获得${rubbishOperRes.Data.ThrowRubbish.ddwCoin}金币\n`
                    );
                  } else {
...
function rubbishOper(dwType, body = "") {
  return new Promise((resolve) => {
    switch (dwType) {
      case "1":
        $.get(
          taskUrl(`story/RubbishOper`, `dwType=1&dwRewardType=0`),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
                console.log(
                  `${$.name} RubbishOper API请求失败，请检查网路重试`
                );
              } else {
                data = JSON.parse(
                  data
      ...
      case "2":
        $.get(
          taskUrl(`story/RubbishOper`, `dwType=2&dwRewardType=0&${body}`),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
...
function awardActTask(function_path, taskInfo = "") {
  ...
  return new Promise((resolve) => {
    switch (function_path) {
      case "Award":
        $.get(
          taskListUrl(function_path, `taskId=${ddwTaskId}`, "jxbfddch"),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
                console.log(
                  `${$.name} awardActTask API请求失败，请检查网路重试`
                );
              } else {
                const {
                  msg,
                  ret,
                  data: { prizeInfo = "" } = {},
                } = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
                let str = "";
                if (msg.indexOf("活动太火爆了") !== -1) {
                  ...
                  str =
                    msg + prizeInfo
                      ? `获得金币 ¥ ${JSON.parse(prizeInfo).ddwCoin}`
                      : "";
                }
                console.log(
                  `【🐮领牛牛任务奖励】${strTaskName} ${str}\n${
                    $.showLog ? data : ""
                  }`
                );
              }
        ...
        $.get(taskUrl(function_path), (err, resp, data) => {
          try {
            if (err) {
              ...
              if (data.iRet === 0 || data.sErrMsg === "success") {
                console.log(
                  `【🐮牛牛任务】开启宝箱成功：获得财富 ¥ ${data.Data.ddwBigReward}\n`
                );
              } else {
      ...
      try {
        if (err) {
          ...
          if (type) {
            ...
            console.log(
              `【${buildNmae}】升级需要${
                data.ddwNextLvlCostCoin
              }金币，保留升级需要的3倍${
                data.ddwNextLvlCostCoin * 3
              }金币，当前拥有${$.info.ddwCoinBalance}金币`
            );
            if (
              data.dwCanLvlUp > 0 &&
              ...
              if (buildLvlUpRes.iRet === 0) {
                console.log(
                  `【${buildNmae}】升级成功：获得${data.ddwLvlRich}财富\n`
                );
              } else {
                console.log(
                  `【${buildNmae}】升级失败：${buildLvlUpRes.sErrMsg}\n`
                );
              }
            } else {
...
function helpByStage(shareCodes) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`story/helpbystage`, `strShareId=${shareCodes}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (data.iRet === 0 || data.sErrMsg === "success") {
              console.log(
                `助力成功：获得${data.Data.GuestPrizeInfo.strPrizeName}`
              );
            } else if (
              data.iRet === 2235 ||
        ...
        $.get(taskListUrl("GetUserTaskStatusList"), async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
              console.log(
                `${$.name} GetUserTaskStatusList 日常任务 API请求失败，请检查网路重试`
              );
            } else {
              const {
                ret,
              ...
              $.allTask = userTaskStatusList.filter(
                (x) => x.awardStatus !== 1 && x.taskCaller === 1
              );
              if ($.allTask.length === 0) {
        ...
        $.get(taskListUrl("GetUserTaskStatusList"), async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
              console.log(
                `${$.name} GetUserTaskStatusList 成就任务 API请求失败，请检查网路重试`
              );
            } else {
              const {
                ret,
              ...
              $.allTask = userTaskStatusList.filter(
                (x) =>
                  x.completedTimes >= x.targetTimes &&
                  x.awardStatus !== 1 &&
                  x.taskCaller === 2
              );
              if ($.allTask.length === 0) {
      ...
      case 1: //成就任务
        for (let i = 0; i < $.allTask.length; i++) {
          ...
          if (taskinfo.completedTimes < taskinfo.targetTimes) {
            console.log(
              `【领成就奖励】${taskinfo.taskName} 该成就任务未达到门槛\n`
            );
          } else {
            //领奖励
...
function doTask(taskId, type = 1) {
  return new Promise(async (resolve) => {
    ...
    $.get(
      taskListUrl(`DoTask`, `taskId=${taskId}`, bizCode),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function awardTask(taskType, taskinfo, bizCode = "jxbfd") {
  return new Promise((resolve) => {
    ...
    switch (taskType) {
      case 0: //日常任务
        $.get(
          taskListUrl(`Award`, `taskId=${taskId}`, bizCode),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
          ...
          try {
            if (err) {
              ...
              if (msg.indexOf("活动太火爆了") !== -1) {
                ...
                console.log(
                  `【领成就奖励】${taskName} 获得财富值 ¥ ${
                    JSON.parse(prizeInfo).ddwMoney
                  }\n${$.showLog ? data : ""}`
                );
              }
            }
      ...
      case 2:
        $.get(
          taskListUrl(`Award`, `taskId=${ddwTaskId}`, `jxbfdprop`),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
                ...
                if (msg.indexOf("活动太火爆了") !== -1) {
                  ...
                  console.log(
                    `【领卡片奖励】${strTaskName} 获得 ${
                      JSON.parse(prizeInfo).CardInfo.CardList[0].strCardName
                    }\n${$.showLog ? data : ""}`
                  );
                }
              }
...
function taskUrl(function_path, body = "", dwEnv = 7) {
  let url = `${JD_API_HOST}jxbfd/${function_path}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=${dwEnv}&_cfd_t=${Date.now()}&ptag=7155.9.47${
    body ? `&${body}` : ""
  }`;
  url += `&_stk=${getStk(url)}`;
  url += `&_ste=1&h5st=${decrypt(Date.now(), "", "", url)}&_=${
    Date.now() + 2
...
function taskListUrl(function_path, body = "", bizCode = "jxbfd") {
  let url = `${JD_API_HOST}newtasksys/newtasksys_front/${function_path}?strZone=jxbfd&bizCode=${bizCode}&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=7155.9.47${
    body ? `&${body}` : ""
  }`;
  url += `&_stk=${getStk(url)}`;
  url += `&_ste=1&h5st=${decrypt(Date.now(), "", "", url)}&_=${
...
function submitCode(myInviteCode, user) {
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://www.jdhelp.cf/jdcodes/submit.php?code=${myInviteCode}&type=jxcfd&user=${user}`,
        timeout: 10000,
      },
      (err, resp, data) => {
        try {
