...
const ACT_ID = "dz2010100034444201",
  shareUuid = "8255cbf3c6cc4b1886faa3a6216f365b";
let ADD_CART = true;
ADD_CART = $.isNode()
...
function getActCk() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("dingzhi/book/develop/activity", `activityId=${ACT_ID}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function getActContent(info = false, shareUuid = "") {
  return new Promise((resolve) => {
    let body = `activityId=${ACT_ID}&pin=${encodeURIComponent(
      $.token
    )}&pinImg=${$.pinImg}&nick=${
      $.nick
    }&cjyxPin=&cjhyPin=&shareUuid=${shareUuid}`;
    $.post(
      taskPostUrl("dingzhi/book/develop/activityContent", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
                  ...
                  for (let task of tasks) {
                    if (["关注店铺"].includes(task.title)) {
                      if (task.okNum < task.dayMaxNum) {
                        ...
                        await doTask(
                          task.settings[0].type,
                          task.settings[0].value
                        );
                      }
                    } else if (
                      ["逛会场", "浏览店铺", "浏览商品"].includes(task.title)
                    ) {
                      if (task.okNum < task.dayMaxNum) {
                        console.log(`去做${task.title}任务`);
                        for (let set of task.settings.filter(
                          (vo) => vo.status === 0
                        )) {
...
function doHelpList(taskType, value) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/taskact/common/getDayShareRecord", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function doTask(taskType, value) {
  let body = `activityId=${ACT_ID}&pin=${encodeURIComponent(
    $.token
  )}&actorUuid=${$.actorUuid}&taskType=${taskType}&taskValue=${value}`;
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/saveTask", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function draw() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/startDraw", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
// 获得图书
function getAllBook() {
  let body = `activityId=${ACT_ID}&actorUuid=${
    $.actorUuid
  }&pin=${encodeURIComponent($.token)}`;
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/getAllBook", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function buyBook(bookUuid, num) {
  let body = `activityId=${ACT_ID}&actorUuid=${
    $.actorUuid
  }&pin=${encodeURIComponent($.token)}&bookUuid=${bookUuid}&buyNum=${num}`;
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/buyBook", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function getMyBook() {
  let body = `activityId=${ACT_ID}&actorUuid=${
    $.actorUuid
  }&pin=${encodeURIComponent($.token)}&type1=1&type2=1&type3=1&type=1`;
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/getMyBook", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function upBook(bookUuid) {
  let body = `activityId=${ACT_ID}&actorUuid=${
    ...
    $.token
  )}&bookUuid=${bookUuid}&isPutOn=1&position=1`;
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/upBook", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
...
function chargeGold() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("dingzhi/book/develop/chargeGold", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
