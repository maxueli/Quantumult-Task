...
const dailyEggUrl =
  "https://active.jd.com/forever/btgoose/?channelLv=yxjh&jrcontainer=h5&jrlogin=true";
const jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
const { JSDOM } = $.isNode() ? require("jsdom") : "";
const { window } = new JSDOM(``, {
  url: dailyEggUrl,
  runScripts: "outside-only",
  pretentToBeVisual: true,
  resources: "usable",
});
...
!(async () => {
  ...
  window.eval(
    await downloadUrl(
      `https://storage.360buyimg.com/rama/common/btgoose/aar.min.js`
    )
  );
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.token =
        (await downloadUrl("https://gia.jd.com/m.html")).match(
          /var\s*?jd_risk_token_id\s*?=\s*["`'](\S*?)["`'];?/
        )?.[1] || "";
      await jdDailyEgg();
    }
  }
...
async function doTask(funcMissionId = null) {
  ...
  taskList = taskList.filter(
    (x) => [-1, 0, 1].includes(x.status) && !errMissionID.includes(x.missionId)
  );
  if (funcMissionId)
    taskList = taskList.filter((x) => x.missionId === funcMissionId);
  for (let task of taskList) {
    ...
    switch (status) {
      case -1:
        ...
        console.log(
          `领任务'${name}'结果：${
            res.code === "0000" ? "成功！" : `失败！原因：${res.msg}`
          }`
        );
        await $.wait(3000);
        if (res.code === "0000") {
      ...
      case 0:
        var tempBody = {};
        Object.assign(tempBody, {
          missionId: doLink.getValByKey("missionId"),
          readTime: +(doLink.getValByKey("readTime") || "-1"),
        });
        if (tempBody.missionId && typeof tempBody.readTime !== -1) {
          res = await doApi("queryMissionReceiveAfterStatus", false, {
            missionId: tempBody.missionId,
          });
          ...
          res = await doApi("getJumpInfo", false, {
            juid: doLink.getValByKey("juid"),
          });
          if (res.code === "0000") {
            await doTask(missionId);
          } else {
      ...
      case 1:
        ...
        msg.push(
          res.code === "0000"
            ? `成功！获得${awardRealNum}${awardName.replace(/^个?/, "个")}`
            : `失败！原因：${res.msg}`
        );
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            if (!$.shareUuid && typeof shareUuid === "string")
              Object.assign($, { shareUuid });
...
async function doApi(
  ...
  switch (functionId) {
    ...
    case "getJumpInfo":
      ...
      preUrl += JSON.stringify(preBody).replace(/"/g, (x) =>
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            console.log(
              "每日首次登陆签到礼包：" + ots(data.resultData.data.signAward)
            );
          } else {
            console.log(`京东服务器返回空数据`);
          }
...
function taskUrl(function_id, body) {
  return {
    url: `${JD_API_HOST}/${function_id}?reqData=${JSON.stringify(body).replace(
      /"/g,
